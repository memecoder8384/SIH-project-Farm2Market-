...
export const MODERN_ITEMS: readonly DockItem[] = [
  { id: "home", label: "Home", icon: <><path d="M2.5 7.5 8 3l5.5 4.5" /><path d="M4 6.8V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V6.8" /><path d="M6.5 14v-4h3v4" /></> },
  { id: "marketplace", label: "Marketplace", icon: <><path d="M2.5 5.5h11l-1 8h-9z" /><path d="M6 5.5v-2a2 2 0 0 1 4 0v2" /></> },
  { id: "farmer", label: "Farmer Area", icon: <><path d="M8 14v-4M8 10a3 3 0 0 1 3-3h2v2a3 3 0 0 1-3 3H8zM8 10a3 3 0 0 0-3-3H3v2a3 3 0 0 0 3 3h2z" /><path d="M5 14h6" /></> },
  { id: "orders", label: "Orders", icon: <><rect x="1.5" y="4.5" width="8.5" height="6.5" rx="1" /><path d="M10 6.5h2.5l2 2v2.5H10" /><circle cx="4.5" cy="12.5" r="1.5" /><circle cx="12" cy="12.5" r="1.5" /></> },
  { id: "pricing", label: "Price Check", icon: <><path d="M8.6 2.2H13v4.4l-6.6 6.6a1.2 1.2 0 0 1-1.7 0L2.2 10.5a1.2 1.2 0 0 1 0-1.7z" /><circle cx="10.6" cy="4.6" r=".9" /></> },
  { id: "ai-demand", label: "Crop Demand", icon: <><path d="M2 12.5l3.5-3.5 2.5 2.5 5-5.5" /><path d="M9.5 6h3.5v3.5" /></> },
];
...
const BRAND_MARK = (
  <svg viewBox="0 0 24 24" aria-hidden="true">
    <rect width="24" height="24" rx="6" fill="#e65c38" />
    <path d="M7 19h10" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" />
    <path d="M12 19V9" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" />
    <path d="M12 9a4 4 0 0 1 4-4h2v2a4 4 0 0 1-4 4h-2" fill="#ffffff" />
    <path d="M12 13a4 4 0 0 0-4-4H6v2a4 4 0 0 0 4 4h2" fill="#ffffff" />
  </svg>
);
...
export function AnimatedTopDock({
  ...
  const items: readonly DockItem[] = props.items ?? (VARIANT_ITEMS[variant] ?? ITEMS);
  ...
  const dockItems = (itemClass: string, iconClass: string, viewBox: string) => items.map((item: DockItem) => (
    <button
      key={item.id}
      className={itemClass}
      data-dock-item
    ...
    return (
      <div className={`animated-top-dock-component atd-modern${className ? ` ${className}` : ""}`}>
        ...
        <header className="atd-modern__bar">
          <a
            ...
            onClick={(event) => {
              ...
              else onItemSelect?.("landing");
            }}
          >
            <span className="atd-modern__mark" aria-hidden="true">{brandMark}</span>
            <span className="atd-modern__word">{brandName}</span>
          </a>
        ...
        <div className="atd-modern__stage" aria-hidden="true">
          <p className="atd-modern__eyebrow">Direct Farm Marketplace</p>
          <p className="atd-modern__headline">Fresh Crops • Safe Payments</p>
        </div>
        <p className="animated-top-dock-component__caption">FARM2MARKET · FRESH CROPS DIRECT FROM LOCAL FARMS</p>
      </div>
    );
  }
