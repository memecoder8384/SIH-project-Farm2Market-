...
import './CrateButton.css';

export interface CrateButtonProps {
  onClick?: () => void;
  cartCount?: number;
  label?: string;
  className?: string;
...
export const CrateButton: React.FC<CrateButtonProps> = ({
  onClick,
  cartCount = 0,
  label = 'HARVEST CRATE',
  className = '',
  ...
  return (
    <div className={`crate-button-wrapper ${className}`.trim()}>
      <button
        type="button"
        onClick={onClick}
        className="crate-button"
        aria-label="Open Harvest Crate"
      >
        ...
        {cartCount > 0 && (
          <span className="crate-badge" title={`${cartCount} items in crate`}>
            {cartCount}
          </span>
        )}
