...
import './MarketplaceLoader.css';

const MARKETPLACE_FACTS = [
  "Buying directly from farmers brings fresh vegetables to your kitchen within 24 hours of harvest.",
  "We test vegetable firmness and freshness naturally for premium culinary quality.",
  "Buying seasonal vegetables supports local farming families and preserves natural soil fertility.",
  "All marketplace vegetables are tested to ensure zero harmful chemical or pesticide sprays.",
  "Direct sales mean vegetables don't sit in cold warehouses for weeks before reaching you.",
  "Scan the QR code on your delivery box to see the farmer's name, farm location, and harvest time.",
  "Every order gives 85% or more of the money directly to the farmer's bank account.",
  "Fresh farm vegetables retain higher dietary fiber, natural vitamins, and minerals."
];

export const MarketplaceLoader: React.FC = () => {
  const [factIndex, setFactIndex] = useState(() =>
    Math.floor(Math.random() * MARKETPLACE_FACTS.length)
  );

  useEffect(() => {
    const interval = setInterval(() => {
      setFactIndex((prev) => (prev + 1) % MARKETPLACE_FACTS.length);
    }, 2000);
    return () => clearInterval(interval);
  }, []);

  return (
    <div className="marketplace-loader-wrapper animate-in fade-in duration-300">
      <svg
        aria-label="loader being flipped clockwise and circled by three curves"
        role="img"
        viewBox="0 0 56 56"
        className="loader"
      >
        <clipPath id="marketplace-sand-mound-top">
          <path
        ...
        <clipPath id="marketplace-sand-mound-bottom">
          <path
      ...
      <div className="mt-8 text-center max-w-lg mx-auto px-4">
        ...
        >
          ...
          <span className="text-stone-700">{MARKETPLACE_FACTS[factIndex]}</span>
        </p>
      </div>
    </div>
  );
