...
import './FarmerFpoLoader.css';

const FPO_FACTS = [
  "When farmers group together, they can buy seeds and tools at up to 25% lower prices.",
  "Farmer groups across India help small farmers sell their crops together for better prices.",
  "Shared cool storage rooms keep fresh vegetables crisp and cool, preventing food waste.",
  "Accurate digital weighing ensures farmers get paid fairly for every single kilogram.",
  "Shared delivery vans help farmers reach city buyers while saving 30% on transport costs.",
  "Selling directly protects farmers from unpredictable price drops in traditional markets.",
  "Advance booking lets farmers know their buyer and agreed price before harvest day.",
  "Direct bank payments reach farmers quickly without middlemen taking big cuts."
];

export const FarmerFpoLoader: React.FC = () => {
  const [factIndex, setFactIndex] = useState(() =>
    Math.floor(Math.random() * FPO_FACTS.length)
  );

  useEffect(() => {
    const interval = setInterval(() => {
      setFactIndex((prev) => (prev + 1) % FPO_FACTS.length);
    }, 2000);
    return () => clearInterval(interval);
  }, []);

  return (
    <div className="farmer-fpo-loader-wrapper animate-in fade-in duration-300">
      <div className="earth">
      ...
      <div className="mt-8 text-center max-w-lg mx-auto px-4">
        ...
        >
          ...
          <span className="text-stone-700">{FPO_FACTS[factIndex]}</span>
        </p>
      </div>
    </div>
  );
...
export default FarmerFpoLoader;
