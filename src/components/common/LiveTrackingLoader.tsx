...
import './LiveTrackingLoader.css';

const LIVE_TRACKING_FACTS = [
  "Keeping vegetables cool at 4°C during delivery doubles how long they stay fresh.",
  "Live temperature monitoring cuts food spoilage during travel from 30% down to under 3%.",
  "Smart road routes save over 4 hours on city deliveries, getting food to you faster.",
  "Solar-powered coolers keep food cold and fresh while saving fuel.",
  "Delivery vans notify you 30 minutes before arrival so you are ready to receive fresh food.",
  "Padded crates ensure tender vegetables and ripe tomatoes arrive completely bruise-free.",
  "Direct farm delivery gets freshly picked vegetables into city kitchens within 12 hours of harvest.",
  "Continuous cool storage keeps your vegetables fresh and crisp from farm to doorstep."
];

export const LiveTrackingLoader: React.FC = () => {
  const [factIndex, setFactIndex] = useState(() =>
    Math.floor(Math.random() * LIVE_TRACKING_FACTS.length)
  );

  useEffect(() => {
    const interval = setInterval(() => {
      setFactIndex((prev) => (prev + 1) % LIVE_TRACKING_FACTS.length);
    }, 2000);
    return () => clearInterval(interval);
  }, []);
  return (
    <div className="live-tracking-loader-wrapper animate-in fade-in duration-300">
      ...
      <div className="mt-8 text-center max-w-lg mx-auto px-4">
        ...
        >
          ...
          <span className="text-stone-700">{LIVE_TRACKING_FACTS[factIndex]}</span>
        </p>
      </div>
    </div>
  );
